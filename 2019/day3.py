...
Strand = collections.namedtuple('Strand', 'dir dist')
...
def parse(wire_str):
    return [Strand(dir=item[0], dist=int(item[1:])) for item in wire_str.split(',')]
...
def points(wire):
    ...
    for s in wire:
        if s.dir == 'R':
            pts += [Point(x=i, y=last.y) for i in range(last.x + 1, last.x + s.dist + 1)]
        elif s.dir == 'L':
            pts += [Point(x=i, y=last.y) for i in range(last.x - 1, last.x - s.dist - 1, -1)]
        elif s.dir == 'U':
            pts += [Point(x=last.x, y=i) for i in range(last.y + 1, last.y + s.dist + 1)]
        elif s.dir == 'D':
            pts += [Point(x=last.x, y=i) for i in range(last.y - 1, last.y - s.dist - 1, -1)]
        else:
            raise Exception("wat" + repr(s))
        last = pts[-1]
...
def run(in1, in2):
    ...
    p1_dists = defaultdict(list)
    p2_dists = defaultdict(list)
    
    for dist, p in enumerate(p1, start=1):
        p1_dists[p].append(dist)
    for dist, p in enumerate(p2, start=1):
        p2_dists[p].append(dist)


    def min_dist(p):
        return min(p1_dists[p]) + min(p2_dists[p])
    print("Part 2:", min_dist(min(intersections, key=min_dist)))
