...
with open('input.txt','r') as f:
    ...
    def init():
        return {'minutes': 0, 'sleeps': defaultdict(int)}
    def worst_time(data):
        worst = max(data['sleeps'].items(), key=lambda x: x[1])
        data['worst_time'] = worst
        return worst
    ...
    for item in data[1:]:
        if item[1] == gtype.SLEEP:
            ...
            for at in range(sleep_start, item[2]):
                guard_data[current_id]['sleeps'][at] += 1
    ...
    thing = worst_time(worst_guard[1])
    print("worst time", thing)

    inception = max(guard_data.items(), key=lambda x: worst_time(x[1])[1])
    print("The other thing: ", inception)
